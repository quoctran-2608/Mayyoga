// ===== Mây Yoga — About Page Share Controls V2 =====
// Progressive enhancement for the "Chia sẻ" block on ve-may-yoga.html.
(function enhanceAboutShare() {
  'use strict';

  function isAboutPage() {
    return /\/ve-may-yoga(?:\.html)?\/?$/.test(window.location.pathname);
  }
  ...
  function injectStyles() {
    if (document.getElementById('may-yoga-about-share-v2')) return;
    var style = document.createElement('style');
    style.id = 'may-yoga-about-share-v2';
    style.textContent = [
      '.article-share.about-share-v2{display:flex!important;align-items:center!important;gap:10px!important;flex-wrap:wrap!important;margin-top:30px!important;padding:13px 15px!important;border:1px solid rgba(61,90,58,.10)!important;border-radius:16px!important;background:linear-gradient(135deg,rgba(247,250,245,.96),rgba(240,245,238,.9))!important}',
      '.about-share-v2 .article-share-label{margin-right:3px;color:#3d5a3a;font-family:"DM Sans",sans-serif;font-size:.9rem;font-weight:650;line-height:1.2}',
      '.about-share-v2 .article-share-btn{position:relative;display:inline-flex!important;align-items:center!important;justify-content:center!important;width:44px!important;height:44px!important;min-width:44px!important;min-height:44px!important;padding:0!important;border:1px solid rgba(255,255,255,.30)!important;border-radius:50%!important;color:#fff!important;text-decoration:none!important;cursor:pointer!important;box-shadow:0 3px 10px rgba(31,61,42,.12)!important;transition:box-shadow .18s ease,filter .18s ease,border-color .18s ease!important}',
      '.about-share-v2 .article-share-btn:hover{filter:brightness(.96);box-shadow:0 6px 16px rgba(31,61,42,.18)!important}',
      '.about-share-v2 .article-share-btn:focus-visible{outline:3px solid rgba(47,111,66,.28)!important;outline-offset:3px!important}',
      '.about-share-v2 .share-facebook{background:#1877f2!important}',
      '.about-share-v2 .share-zalo{background:#0068ff!important}',
      '.about-share-v2 .share-x{background:#111!important}',
      '.about-share-v2 .share-native{background:#397334!important}',
      '.about-share-v2 .share-copy{background:#5a7a56!important}',
      '.about-share-v2 .article-share-btn svg{display:block;pointer-events:none}',
      '.about-share-v2 .article-share-status{min-width:0;color:#557057;font-size:.78rem;font-weight:600;line-height:1.3}',
      '@media(max-width:480px){.article-share.about-share-v2{gap:9px!important;padding:13px!important}.about-share-v2 .article-share-label{flex:0 0 100%;margin:0 0 2px}.about-share-v2 .article-share-status{flex:0 0 100%;margin-top:1px}}',
      '@media(prefers-reduced-motion:reduce){.about-share-v2 .article-share-btn{transition:none!important}}'
    ].join('\n');
  ...
  function init() {
    ...
    if (!share || share.dataset.shareV2 === 'true') return;

    injectStyles();

    var canonical = document.querySelector('link[rel="canonical"]');
    var pageUrl = canonical && canonical.href ? canonical.href : window.location.href.split('#')[0];
    var ogTitle = document.querySelector('meta[property="og:title"]');
    var pageTitle = ogTitle && ogTitle.content ? ogTitle.content : document.title;
    var encodedUrl = encodeURIComponent(pageUrl);
    var encodedTitle = encodeURIComponent(pageTitle);

    share.classList.add('about-share-v2');
    share.dataset.shareV2 = 'true';
    share.setAttribute('aria-label', 'Chia sẻ trang Về Mây Yoga');
    share.innerHTML = [
      ...
      nativeButton.addEventListener('click', async function() {
        try {
          await navigator.share({
            title: pageTitle,
            text: 'Tìm hiểu về Mây Yoga và hành trình Hatha Yoga.',
            url: pageUrl
          });
        } catch (error) {
